...
export function DashboardTech() {
  ...
  const [confirmedProspects, setConfirmedProspects] = useState<ProjectProspect[]>([]);
  const [projectsInProgress, setProjectsInProgress] = useState<ProjectProspect[]>([]);
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Calculate stats
        const inProgress = prospects.filter(p => p.status === 'negotiation').length;
        const confirmed = prospects.filter(p => p.status === 'qualified').length;

        // Pipeline revenue calculation (simplified - would need actual deal amounts)
        const pipelineRevenue = prospects
          .filter(p => p.status === 'qualified' || p.status === 'negotiation' || p.status === 'proposal')
        ...
        const confirmedList = prospects
          ...
          .map(p => ({
            ...
            techStack: ['React', 'Node.js', 'PostgreSQL'].slice(0, Math.floor(Math.random() * 3) + 1),
          }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">{formatCurrency(stats.pipelineRevenue)}</div>
            <p className="text-xs text-muted-foreground mt-1">
              Revenus potentiels
            </p>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Prêts à démarrer le développement
          </CardDescription>
        </CardHeader>
        <CardContent>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div className="space-y-1 flex-1">
                        <div className="flex items-center gap-2">
                          <h4 className="font-medium text-sm">{project.contact_name}</h4>
                ...
                {upcomingDeadlines.map(item => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start justify-between">
                        <div className="space-y-1 flex-1">
                          <h4 className="font-medium text-sm">{item.contact_name}</h4>
                          {item.company_name && (
      ...
      <Card>
        <CardContent className="flex items-center justify-center gap-4 py-8">
          <Button size="lg" asChild>
            <Link href="/prospects?status=qualified">Voir tous les besoins confirmés</Link>
          </Button>
